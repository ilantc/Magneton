function [ cleanConf ] = cleanConfigurations( targets_conf,targets,BEGIN_COL,END_COL,DURATION_COL,max_target_per_drone,maxFlightTime,verbose)
    ...
    for i=1:C;
        ...
        for j=1:size(targets_options,1)
            ...
            if ( (details(size(curr_option,2),REAL_END_COL_details) - details(1,REAL_BEGIN_COL_details)) > maxFlightTime)
                verbose && fprintf('Max flight is %d, exceeded max flight time,i=%d, j= %d,\n start is %d, end is %d\n',maxFlightTime,i,j,details(1,REAL_BEGIN_COL_details),details(size(curr_option,2),REAL_END_COL_details));
                targets_options_checked(j) = 0;
            end
