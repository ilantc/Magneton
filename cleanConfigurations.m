function [ cleanConf ] = cleanConfigurations( targets_conf,targets,BEGIN_COL,END_COL,DURATION_COL,verbose)
    ...
    for i=1:C;
        ...
        for j=1:size(targets_options,1);
            ...
            details=zeros(size(option),5); % begin, end, duration, real_begin, real_end
            %details table will describe for each target the above info%

            for op=1:size(option);
                details(op,BEGIN_COL_details)=targets(option(op),BEGIN_COL);
                details(op,END_COL_details)=targets(option(op),END_COL);
                details(op,DURATION_COL_details)=targets(option(op),DURATION_COL);
            end

            details(1,REAL_END_COL_details)=details(1,BEGIN_COL_details)+details(1,DURATION_COL_detail)+
            
            for k=2:max_target_per_drone;
